# Using RimWorld Loading Optimizer

RimWorld Loading Optimizer avoids repeated work during startup. With a supported game and dependency build, enabling the mod now enables its established improvements without special launch arguments. This is an unpublished development build with deliberately narrow compatibility; a matching version number alone does not establish support.
...
Explicit development launch arguments override these settings. Ordinary users do not need them. Remove old development arguments before relying on the settings; `--rlo-bypass` remains an emergency way to skip activation for a launch.

## Supported-build policy

RLO checks actual game and dependency code, including file identity and the methods modified by other mods. This is stricter than comparing displayed version numbers. An unrecognized version uses ordinary behavior rather than an unverified replacement. A new game or supplier version requires focused identity, behavior and startup checks before it is added to the support list.
...
Improvement depends on which work your modlist performs. Cached XML can already skip the searches RLO accelerates, and DDS-heavy lists may do no PNG processing. Separate feature percentages cannot be added. See the [measured comparisons](feature-benchmark-results.md).
