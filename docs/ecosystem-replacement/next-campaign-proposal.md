...
```mermaid
flowchart LR
  A[Native source selection] --> B[Owned input snapshot]
  B --> C{Valid reusable generation?}
  C -->|yes| D[Private restored data]
  C -->|no| E[Native or equivalent producer]
  E --> D
  D --> F[Validate complete result]
  F --> G[Publish at native consumer boundary]
  E --> H[Stage next cache generation]
  H --> I[Atomic manifest publication]
  F -->|failure before publication| J[Ordinary native path]
```
