...
> Historical state snapshot from private main `5299ff4`, archived 9 September 2026. See [current state](../current-state.md) for the active roadmap and source mismatch.
...
Read the [user guide and supported-build policy](../user-guide.md) before normal installation. This remains a development build with exact Windows GOG/dependency contracts; the Workshop upload does not establish latest-Steam or universal 1.6 support. Earlier identities and measurements are in [development checkpoints](development-checkpoints.md).
