...
Normal activation uses in-game settings and the existing exact game, Harmony and optional supplier compatibility checks. Supported searches and the established optional improvements default on; processed-PNG caching stays opt-in because construction can make the first launch slower. Changes require restart. Explicit development selectors retain precedence, including bypass, baseline and malformed selections.

The HugsLib mechanism was recovered from `d0ee269`, adapting names and current instruction comparison while preserving the original lazy report and optional Performance Optimizer compatibility. The earlier experiment was parked for former scope restrictions, not a completed negative comparison. Its current measured cost did not justify completing long timing pairs; the prototype was removed as recorded below.

The larger workload will use the existing sorted mixed selection with the retained optional suppliers included where compatible. Preserve memory admission and the other application's state; no larger list will be used to rescue a weak candidate. Reuse existing receipts and a bounded menu observation to identify remaining cost, rather than adding a new profiling framework.

Validation and final outcomes are recorded here as the work completes. Source changes, package identity, fixture deployment, startup timing and gameplay coverage remain separate claims.
...
The original report's complete measured cost is below 0.2% of this cached startup, before replacement and installation costs. That ceiling does not justify four more long timing runs or retention on this workload. The prototype, dedicated tests and selector are removed. This is a current cost-based stopping decision, not a measured end-to-end loss or a disproof of the larger historical workload. No larger selection will be used to rescue it.

## Completed normal activation and compatibility checks
...
This is meaningful new-colony/save-reload coverage, not mounted combat, every preset interaction, prolonged gameplay, latest-Steam or cross-platform qualification. PNG rendering retains its existing separate qualification; PNG caching remains off in these native DDS-source checks.
...
|---|---:|---:|
| Completed menu, seconds | 113.453048 | 60.276140 |
| First menu-drawing call, seconds | 2.776367 | 3.448742 |
...
The stable fresh capture records approximately 9.96 seconds of XML patching, 5.71 seconds of XML conversion, 5.33 seconds of texture-holder loading, 3.69 seconds of audio loading, 8.15 seconds of mod constructors and 21.61 seconds of static initialization. These scopes overlap and are not additive. Giddy-Up accounts for 18.21 seconds of that static initialization, including its 17.80-second offset scope. Its much smaller warm scope, while the foreground inventory changed, demonstrates variability; it does not establish a permanent removable 18-second budget or isolate GPU versus other host effects.

Other measured mod constructors were smaller: Vanilla Expanded Framework about 1.69 seconds, Android races 1.00 second, Combat Extended 0.87 seconds and Geological Landforms 0.54 seconds. Those are full original callbacks, not identified waste. The prior approximately 64-second late-menu observation used a different larger workload, so the present 2.78/3.45-second calls must not be presented as a matched 60-second saving.

**Decision:** no late shared-search opportunity is demonstrated, and no new general replacement is justified. The current evidence supports product delivery and targeted compatibility work over further broad optimization hunting. A future Giddy-Up/graphics investigation would first need to establish the cause and repeatability of the remaining long readback scope under the intended use conditions; existing warm DDS/cache failures are not reopened. Detailed per-mod costs and receipts are in `artifacts/product-closeout/residual-analysis.json`.

## Final identities and state

- Runtime/package source: `4422f6222a54c365618e154de66ba88ee048ab6f`.
- Runtime DLL SHA-256: `722db104891f7d0c1add568684cefb3eca201b6e8bd839ef4b2a7bf05aaefec3`.
- Package: `artifacts/op7-fixture-package/4422f6222a54c365618e154de66ba88ee048ab6f`.
- Last capture: `close-r02-user-warm`, normal exit and capture at 2026-09-08 00:53:18 UTC (September 7 locally); environment change excludes a timing comparison.
- Original Character Editor, EyePatch and diagnostic-mod exclusions restored; final fixture audit passed; no fixture game remains running. The last captured run is preserved, with no new prepared launch after restoration.
- Local integration only. Normal Steam/Workshop/profile/save/cache state was untouched. No push, publication or licensing grant.

Private source inspection, helpers, build logs, raw captures and comparison summaries remain under ignored `artifacts/product-closeout/` and `.rlo-test-instance/results/`. Documentation-only closeout does not change the tested runtime or observer package.
