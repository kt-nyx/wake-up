...
internal sealed class PngCache
{
    ...
    internal void Invalidate(string key)
    {
        try
        {
            ...
            File.Delete(file);
            stored = Math.Max(0, stored - length);
        }
    ...
    internal Entry? Read(string key)
    {
        try
        {
            ...
            if (new FileInfo(file).Length > MaximumEntry + 256)
    ...
    internal void Write(string key, Entry entry)
    {
        ...
        if (!CanCapture(entry.Pixels.Length) || File.Exists(file))
            return;
        try
        {
            ...
            using (var writer = new BinaryWriter(File.Create(pending)))
            {
