...
internal static class StartupLaunchSelector
{
    internal static StartupLaunchDecision Parse(IReadOnlyList<string> arguments)
    {
        ...
        for (int index = 0; index < arguments.Count; index++)
        {
            ...
            if (string.Equals(argument, "--wake-up-reference", StringComparison.Ordinal)
                || string.Equals(argument, "--wake-up-bypass", StringComparison.Ordinal))
            {
                reference = true;
            }

            if (argument.StartsWith("--wake-up-mode=", StringComparison.Ordinal))
            {
                string value = argument.Substring("--wake-up-mode=".Length);
