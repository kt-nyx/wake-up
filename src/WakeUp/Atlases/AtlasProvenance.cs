...
internal static class AtlasProvenance
{
    internal static string Revision(Texture2D source)
    {
        if (!UnityData.IsInMainThread || source == null || source.width >= 512 || source.height >= 512
            || source.width < 1 || source.height < 1 || !SystemInfo.supportsAsyncGPUReadback)
            throw new NotSupportedException("atlas-input-snapshot");
        var descriptor = PngRuntime.Describe(source, Array.Empty<byte>());
        using var bytes = new MemoryStream();
        using var writer = new BinaryWriter(bytes, Encoding.UTF8, true);
        writer.Write("atlas-gpu-producer-revision-v1");
        writer.Write(descriptor.Width); writer.Write(descriptor.Height); writer.Write(descriptor.TextureFormat);
        writer.Write(descriptor.GraphicsFormat); writer.Write(descriptor.Mips); writer.Write(descriptor.Readable);
        writer.Write(descriptor.Filter); writer.Write(descriptor.WrapU); writer.Write(descriptor.WrapV);
        writer.Write(descriptor.WrapW); writer.Write(descriptor.Aniso); writer.Write(descriptor.Bias);
        writer.Write(source.minimumMipmapLevel); writer.Write(source.requestedMipmapLevel);
        for (int mip = 0; mip < source.mipmapCount; mip++)
        {
            ...
            try
            {
                ...
                writer.Write(PngCache.Hash(data));
            }
            catch (Exception e)
        ...
        writer.Flush(); return PngCache.Hex(PngCache.Hash(bytes.ToArray()));
    }
}
