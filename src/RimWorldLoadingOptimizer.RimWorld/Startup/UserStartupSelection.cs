...
internal static class UserStartupSelection
{
    ...
    internal static string[] Resolve(IReadOnlyList<string> arguments, OptimizerSettings settings, string saveDataRoot)
    {
        ...
        if (!settings.DefinitionSearches)
            selected.Add("--rlo-user-def-search=off");
        if (!settings.TypeSearches)
            selected.Add("--rlo-user-type-search=off");
        if (settings.GagarinReuse)
            selected.Add("--rlo-gagarin-cache=on");
        if (settings.CharacterPresets)
            selected.Add("--rlo-character-presets=on");
        if (settings.LoadingProgress)
            selected.Add("--rlo-loading-progress=coalesce");
        if (settings.GiddyTextures)
            selected.Add("--rlo-giddy-textures=on");
        if (settings.PngCache)
            selected.Add("--rlo-png=cache");
