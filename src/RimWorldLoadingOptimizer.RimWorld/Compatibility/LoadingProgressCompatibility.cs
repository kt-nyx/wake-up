...
internal static class LoadingProgressCompatibility
{
    ...
    internal static bool TryInitialize()
    {
        ...
        try
        {
            ...
            if (assembly.ManifestModule.ModuleVersionId != new Guid("d5765fe2-129d-4df2-a914-5b48feef8562")) return false;
            using (var file = File.OpenRead(Path.Combine(mod.RootDir, "1.6", "Assemblies", "ilyvion.LoadingProgress.dll")))
                if (Hash(file) != "4F19F836FD41C8E702295CB90D7D5E66C53F38CDA8A13A3ED79F1D063E1AFCEF") return false;
    ...
    private static string Hash(Stream stream) { using var hash = SHA256.Create(); return BitConverter.ToString(hash.ComputeHash(stream)).Replace("-", ""); }
}
