...
internal static class XmlReuseContract
{
    ...
    internal static readonly string[] Bodies = {
        "AFFADF2EAB534A9F8749073DE278D086FB0D45345AB31086480330BC3750AD02",
        "DEE6FE6B3D5C98234CB4A8385134DF234E3EB582E2B81FBD3E863D21CDE0C789",
    ...
    internal static bool Allows(out string reason)
    {
        ...
        try
        {
            ...
            if (!AllowsPatches(AccessTools.Method(typeof(LoadedModManager), "ParseAndProcessXML"))
                || Harmony.GetAllPatchedMethods().Any(m => m.DeclaringType?.Assembly == typeof(System.Xml.XmlDocument).Assembly))
            { reason = "foreign-xml-consumer-or-library-hook"; return false; }
            for (int i = 0; i < methods.Length; i++)
            {
                ...
                if (!AllowsPatches(methods[i])) { reason = "foreign-xml-hook-" + methods[i].Name; return false; }
            }
