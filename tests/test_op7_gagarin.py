...
spec = importlib.util.spec_from_file_location("gagarin_fixture", Path(__file__).resolve().parents[1] / "scripts/op7_fixture.py")
...
class GagarinSelectionTests(unittest.TestCase):
    def test_only_cache_selector_changes_between_arms(self):
        ...
        self.assertEqual([x for x in control if not x.startswith("--rlo-gagarin-cache=")],
                         [x for x in candidate if not x.startswith("--rlo-gagarin-cache=")])
        self.assertIn("--rlo-strategy=startup-searches", candidate)
        self.assertIn("--rlo-gagarin-cache=on", candidate)
        self.assertNotIn("--rlo-gagarin-cache=on", f.run_arguments(root, "absent"))
