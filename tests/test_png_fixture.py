...
spec = importlib.util.spec_from_file_location('png_fixture', Path(__file__).resolve().parents[1] / 'scripts/fixture.py')
...
class PngSelectionTests(unittest.TestCase):
    def test_explicit_comparison_and_ordinary_default(self):
        ...
        self.assertEqual([s for s in control if not s.startswith('--wake-up-png=')], [s for s in trial if not s.startswith('--wake-up-png=')])
