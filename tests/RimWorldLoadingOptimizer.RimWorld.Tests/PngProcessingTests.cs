...
public sealed class PngProcessingTests
{
    ...
    public void NativeHashMatchesSha256WithoutChangingInput()
    {
        ...
        if(Environment.OSVersion.Platform==PlatformID.Win32NT) Assert.That(PngCache.NativeHashCalls,Is.GreaterThanOrEqualTo(5));
    }
    [Test]
    public void PinnedOriginalBodiesAndPngCallsitesAreAvailable()
    {
        ...
        foreach (var method in PngRuntime.Targets)
        {
            ...
            // Its original-PE hash is independently checked by the ignored metadata
            // helper, which reproduces all four loader hashes, and by live admission.
            if (method == PngRuntime.Iterator) continue;
            Assert.That(SemanticMethodIdentity.TryHash(method, out string hash, out string reason), Is.True, reason);
            lines.Add(method.Name + " " + hash);
            Assert.That(hash, Is.EqualTo(PngRuntime.Expected[method.Name]));
        }
        File.WriteAllLines(@"Z:\Development\Large Projects\RimWorld Loading Optimizer\artifacts\png-processing\original-hashes.txt", lines);
        ...
        {
            if (!(original[i].operand is MethodInfo before) || !(replaced[i].operand is MethodInfo after) || before==after) continue;
    ...
    {
        string root=Path.Combine(Path.GetTempPath(),"rlo-png-test-"+Guid.NewGuid().ToString("N"));
        try
        {
            ...
            cache.Write(key,entry); var read=cache.Read(key);
            Assert.That(read,Is.Not.Null); Assert.That(read!.Pixels,Is.EqualTo(entry.Pixels)); Assert.That(read.Mips,Is.EqualTo(3));
            Assert.That(cache.Read(PngCache.Hex(PngCache.Hash(new byte[]{1,2,4}))),Is.Null);
            string file=Path.Combine(root,key+".bin"); byte[] damaged=File.ReadAllBytes(file); damaged[damaged.Length-1]^=1; File.WriteAllBytes(file,damaged);
            Assert.That(cache.Read(key),Is.Null); Assert.That(cache.Errors,Is.EqualTo(1));
            cache.Write(key,entry); Assert.That(cache.Read(key)!.Pixels,Is.EqualTo(entry.Pixels));
            ...
            cache.Invalidate(key); Assert.That(cache.Read(key),Is.Null);
        }
