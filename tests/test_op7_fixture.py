...
spec = importlib.util.spec_from_file_location("op7_fixture", Path(__file__).parents[1] / "scripts/op7_fixture.py")
...
class FixtureTests(unittest.TestCase):
    def test_user_activation_has_no_optimizer_selectors_and_rejects_mixed_controls(self):
        ...
        self.assertFalse(any(a.startswith('--rlo-') for a in args))
    ...
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="rlo-fixture-test-")
    ...
    def test_offline_initialization_order_integrity_reset_and_refresh(self):
        ...
        with patch.object(f, "discover", return_value=sources), patch.object(f, "git", return_value="a" * 40), patch.object(f, "DEVELOPMENT_GAME", f.digest(game / "RimWorldWin64_Data/Managed/Assembly-CSharp.dll")):
            ...
            for name in ["RimWorld"]:
                (candidate / "Assemblies" / ("RimWorldLoadingOptimizer."+name+".dll")).write_bytes(b"offline mock")
            ...
            with patch.object(f.subprocess, "Popen") as launch:
                ...
                self.assertEqual(verified["command"], [str(game / "RimWorldWin64.exe"),
                    ...
                    "--rlo-op7=candidate", "--rlo-strategy=startup-searches", "--rlo-giddy-textures=on", "--rlo-character-presets=on", "-logFile", str(self.root / "profile/Player.log")])
            ...
            self.assertFalse(any(a.startswith("--rlo-observe=") for a in verified["command"]))
            self.assertIn("--rlo-strategy=type-lookup", verified["command"])
            self.assertFalse(any(a.startswith("--rlo-workers=") for a in verified["command"]))
    ...
    def test_gagarin_cache_requires_matching_capture_and_recorded_bytes(self):
        ...
        with patch.object(f, "audit", return_value={"deployedRuntimeMatches": True}), \
            ...
            self.assertEqual((self.root / "profile/MissileGirl/cache.bin").read_bytes(), b"foreign cache")
            self.assertFalse((self.root / "profile/Config/RimWorldLoadingOptimizer").exists())
    ...
    def test_default_preparation_automates_menu_exit_with_explicit_manual_opt_out(self):
        ...
        with patch.object(f, "audit", return_value={"deployedRuntimeMatches": True}), \
            ...
            self.assertFalse(any(arg.startswith("--rlo-") for arg in run["arguments"]))
    ...
    def test_timing_arguments_never_add_optimizer_to_absent_run(self):
        ...
        for mode in ("baseline", "candidate"):
            ...
            self.assertIn("--rlo-strategy=startup-searches", args)
            self.assertIn("--rlo-op7=" + mode, args)
    ...
    def test_giddy_textures_are_optional_and_candidate_only(self):
        self.assertNotIn("--rlo-giddy-textures=on", f.run_arguments(self.root, "candidate"))
        for selection in ("on", "timing", "verify"):
            self.assertIn("--rlo-giddy-textures=" + selection, f.run_arguments(self.root, "candidate", giddy_textures=selection))
    ...
    def test_character_presets_are_optional_and_candidate_only(self):
        self.assertNotIn("--rlo-character-presets=on", f.run_arguments(self.root, "candidate"))
        for selection in ("on", "timing"):
            self.assertIn("--rlo-character-presets=" + selection,
