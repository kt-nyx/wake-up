...
"""Permanent Windows DRM-free OP7 fixture. Standard-library Python 3.11+.
...
def discover():
    ...
    require(PACKAGE_ID not in pids, "Source contains RLO validation package; resolve before freezing")
...
def initialize(root, refresh=False, resume=None):
    ...
    manifest = {"schema": "rlo-drm-fixture.v1", "generation": gid, "createdUtc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                ...
                "excludedProfilePolicy": "No saves, presets, generated caches, logs or RLO state. Applicable Mod_<folder> XML, startup preferences, Config settings subfolders and HugsLib/ModSettings.xml only."}
...
def run_order(manifest, lane, mode, menu_observer=False, excluded_ids=(), selection=None):
    require(lane in ("representative", "activation", "xml-expanded"), "Unknown OP7 lane")
    require(mode in ("absent", "baseline", "candidate"), "Unknown OP7 mode")
...
def run_arguments(root, mode, observation="timing", strategy="startup-searches",
    ...
    if activation == "user":
        return args
    if mode != "absent":
        args.extend(["--rlo-op7=" + mode, "--rlo-strategy=" + strategy])
    if gagarin_cache != "off":
        args.append("--rlo-gagarin-cache=" + gagarin_cache)
    if png != "off":
        args.append("--rlo-png=" + png)
    if giddy_textures != "off":
        args.append("--rlo-giddy-textures=" + giddy_textures)
    if character_presets != "off":
        args.append("--rlo-character-presets=" + character_presets)
    if loading_progress != "off":
        args.append("--rlo-loading-progress=" + loading_progress)
    if png_source != "native":
        args.append("--rlo-png-source=" + png_source)
    return args
...
def candidate_matches_snapshot(root, manifest, generation):
    ...
    if {row["path"] for row in candidate.get("files", []) if row["kind"] == "file"} != {
            "About/About.xml", "Assemblies/RimWorldLoadingOptimizer.RimWorld.dll"}:
        return False
...
def reference_environment(root):
    ...
    return dict(os.environ, RLO_RIMWORLD_MANAGED_DIR=str(managed),
               RLO_PREPATCHER_ASSEMBLIES_DIR=str(refs), RLO_REFERENCE_TARGET="gog-rev573")
...
def build(root):
    ...
    require(not git("status", "--porcelain", "--", "src", "build", "scripts/op7_fixture.py", "Directory.Build.props", "Directory.Packages.props", "global.json", "NuGet.config", "validation/op7-package"), "Uncommitted build inputs")
    env = reference_environment(root)
    refs = Path(env["RLO_PREPATCHER_ASSEMBLIES_DIR"])
    # Verifier checks exact approved build references; runtime admission stays separate.
    product = "src/RimWorldLoadingOptimizer.RimWorld/RimWorldLoadingOptimizer.RimWorld.csproj"
    for command in [["pwsh", "-NoProfile", "-File", str(REPO / "build/Verify-LocalReferences.ps1")],
        ...
        subprocess.run(command, cwd=REPO, env=env, check=True)
    package_root = REPO / "artifacts/op7-fixture-package" / revision
    ...
    shutil.copy2(REPO / "validation/op7-package/About/About.xml", package_root / "About/About.xml")
    for name, framework in [("RimWorld", "net472")]:
        filename = f"RimWorldLoadingOptimizer.{name}.dll"
        shutil.copy2(REPO / f"artifacts/bin/RimWorldLoadingOptimizer.{name}/Release/{framework}" / filename, package_root / "Assemblies" / filename)
    check_script = r'''
foreach ($name in @('RimWorld')) {
    $expected = 'RimWorldLoadingOptimizer.' + $name
    $path = Join-Path $env:RLO_PACKAGE_CHECK_ROOT ('Assemblies/' + $expected + '.dll')
    ...
    if ($identity.Name -cne $expected -or $identity.Version.ToString() -cne '1.0.0.0' -or
        $version.ProductVersion -cne ('1.0.0+' + $env:RLO_PACKAGE_CHECK_REVISION)) {
...
'''
    subprocess.run(["pwsh", "-NoProfile", "-Command", "$ErrorActionPreference='Stop';" + check_script],
                   env=dict(env, RLO_PACKAGE_CHECK_ROOT=str(package_root), RLO_PACKAGE_CHECK_REVISION=revision), check=True)
    receipt = {"sourceRevision": revision, "referenceTarget": "gog-rev573",
        ...
        str(Path(env["RLO_RIMWORLD_MANAGED_DIR"]) / "Assembly-CSharp.dll"): digest(Path(env["RLO_RIMWORLD_MANAGED_DIR"]) / "Assembly-CSharp.dll"),
...
def deploy(root, package_root):
    ...
    require({r["path"] for r in receipt["files"] if r["kind"] == "file"} == {
        "About/About.xml", "Assemblies/RimWorldLoadingOptimizer.RimWorld.dll"}, "Candidate must contain only About.xml and the retained runtime DLL; stale Core and third-party assemblies are refused")
...
def build_menu_observer(root):
    revision = git("rev-parse", "HEAD")
    require(not git("status", "--porcelain", "--", "validation/menu-observer", "scripts/op7_fixture.py",
...
def prepare(root, mode, label, lane="representative", observation="timing", strategy="startup-searches",
    ...
    if png_cache_from:
        ...
        prefix = "RimWorldLoadingOptimizer/PngCache/"
        ...
        png_cached = inside(root, previous_folder / "profile/RimWorldLoadingOptimizer/PngCache")
    ...
    if png_cached is not None:
        shutil.copytree(png_cached, staged_profile / "RimWorldLoadingOptimizer/PngCache")
        verify_tree(staged_profile / "RimWorldLoadingOptimizer/PngCache", png_rows, True)
    ...
    run = {"schema": "rlo-drm-run.v1", "label": label, "mode": mode, "lane": lane,
           ...
           "restoredCacheKinds": (["gagarin-MissileGirl"] if foreign_cached is not None else []) + (["rlo-PngCache"] if png_cached is not None else []),
    ...
    return {"prepared": label, "mode": mode, "transaction": tid, "preflight": check,
            "command": f'python "{REPO / "scripts/op7_fixture.py"}" launch --label {label} --authorize-live-launch'}
...
def restore_profile(root):
    ...
    return {"restored": str(root / "profile"), "transaction": tid, "activeOrder": "exact source order; RLO not enabled"}
...
class StartupLogObserver:
    """Bounded incremental external timing, identical with and without RLO."""
...
def main():
    ...
    parser.add_argument("--activation", choices=["fixture", "user"], default="fixture", help="User exercises normal activation without RLO command-line selectors")
